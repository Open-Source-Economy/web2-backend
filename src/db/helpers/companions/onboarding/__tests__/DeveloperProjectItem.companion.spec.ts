...
describe("DeveloperProjectItemCompanion.mergeRolesAndRights", () => {
  describe("when there are no changes", () => {
    it("should return hasChanges=false when roles and rights are identical", () => {
      const existingRoles = [DeveloperRoleType.FOUNDER, DeveloperRoleType.PROJECT_LEAD];
      const existingRights = [MergeRightsType.FULL_COMMITTER];
      const newRoles = [DeveloperRoleType.FOUNDER, DeveloperRoleType.PROJECT_LEAD];
      ...
      expect(result.mergedMergeRights).toEqual([MergeRightsType.FULL_COMMITTER]);
    });
    ...
    it("should return hasChanges=false when new items are subset of existing", () => {
      const existingRoles = [DeveloperRoleType.FOUNDER, DeveloperRoleType.PROJECT_LEAD, DeveloperRoleType.MAINTAINER];
      const existingRights = [MergeRightsType.FULL_COMMITTER, MergeRightsType.REVIEWER];
  ...
  describe("when there are changes", () => {
    it("should add new roles only", () => {
      const existingRoles = [DeveloperRoleType.FOUNDER];
      const existingRights = [MergeRightsType.FULL_COMMITTER];
      const newRoles = [DeveloperRoleType.PROJECT_LEAD, DeveloperRoleType.MAINTAINER];
      const newRights = [MergeRightsType.FULL_COMMITTER];
      ...
      expect(result.addedRoles).toEqual([DeveloperRoleType.PROJECT_LEAD, DeveloperRoleType.MAINTAINER]);
      expect(result.addedMergeRights).toEqual([]);
      expect(result.mergedRoles).toHaveLength(3);
      expect(result.mergedRoles).toContain(DeveloperRoleType.FOUNDER);
      expect(result.mergedRoles).toContain(DeveloperRoleType.PROJECT_LEAD);
    ...
    it("should add new merge rights only", () => {
      ...
      const existingRights = [MergeRightsType.FULL_COMMITTER];
      const newRoles = [DeveloperRoleType.FOUNDER];
      const newRights = [MergeRightsType.REVIEWER, MergeRightsType.RELEASE_MANAGER];
      ...
      expect(result.hasChanges).toBe(true);
      expect(result.addedRoles).toEqual([]);
      expect(result.addedMergeRights).toEqual([MergeRightsType.REVIEWER, MergeRightsType.RELEASE_MANAGER]);
      expect(result.mergedRoles).toEqual([DeveloperRoleType.FOUNDER]);
      expect(result.mergedMergeRights).toHaveLength(3);
      expect(result.mergedMergeRights).toContain(MergeRightsType.FULL_COMMITTER);
      expect(result.mergedMergeRights).toContain(MergeRightsType.REVIEWER);
      expect(result.mergedMergeRights).toContain(MergeRightsType.RELEASE_MANAGER);
    });
  ...
  describe("edge cases", () => {
    it("should handle duplicates in new arrays", () => {
      ...
      expect(result.addedRoles).toEqual([DeveloperRoleType.PROJECT_LEAD, DeveloperRoleType.PROJECT_LEAD]); // Filter doesn't dedupe input
      expect(result.addedMergeRights).toEqual([MergeRightsType.REVIEWER, MergeRightsType.REVIEWER]);
    ...
    it("should handle partial overlap", () => {
      const existingRoles = [DeveloperRoleType.FOUNDER, DeveloperRoleType.MAINTAINER];
      const existingRights = [MergeRightsType.FULL_COMMITTER];
      const newRoles = [DeveloperRoleType.FOUNDER, DeveloperRoleType.PROJECT_LEAD]; // one overlap, one new
      const newRights = [MergeRightsType.FULL_COMMITTER, MergeRightsType.REVIEWER]; // one overlap, one new
