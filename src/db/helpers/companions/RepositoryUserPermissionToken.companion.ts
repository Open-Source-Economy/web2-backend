...
export namespace RepositoryUserPermissionTokenCompanion {
  export function fromBackend(row: any, table_prefix: string = ""): RepositoryUserPermissionToken | ValidationError {
    ...
    const userGithubOwnerLogin = validator.requiredString(`${table_prefix}user_github_owner_login`);
    const token = validator.requiredString(`${table_prefix}token`);
    const repositoryId = RepositoryIdCompanion.fromBackendForeignKey(row, table_prefix);
    ...
    const hasBeenUsed = validator.requiredBoolean(`${table_prefix}has_been_used`);
