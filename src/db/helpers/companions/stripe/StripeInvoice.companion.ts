...
export namespace StripeInvoiceCompanion {
  export function fromBackend(
    ...
    const accountCountry = validator.requiredString(
      `${table_prefix}account_country`,
    );
    ...
    const totalExclTax = validator.requiredNumber(
      `${table_prefix}total_excl_tax`,
    );
    const subtotal = validator.requiredNumber(`${table_prefix}subtotal`);
    const subtotalExclTax = validator.requiredNumber(
      `${table_prefix}subtotal_excl_tax`,
    );
    const hostedInvoiceUrl = validator.requiredString(
      `${table_prefix}hosted_invoice_url`,
    );
