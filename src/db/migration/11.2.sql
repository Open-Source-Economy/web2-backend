...
----------------------------------------------------------------------------------
----------------------------- EXPECTED RESULT ------------------------------------
-- the new `project` table that serves as a container for polymorphic items, allowing for different types of project items (e.g., GitHub repositories, owners, URLs) to be stored in a single table.
----------------------------------------------------------------------------------
...
--             (item_type = 'GITHUB_REPOSITORY' AND github_owner_login IS NOT NULL AND github_repository_name IS NOT NULL AND url IS NULL) OR
--             (item_type = 'GITHUB_OWNER'      AND github_owner_login IS NOT NULL AND github_repository_name IS NULL     AND url IS NULL) OR
--             (item_type = 'URL'               AND url IS NOT NULL AND github_owner_login IS NULL AND github_repository_name IS NULL)
--         )
-- );
...
INSERT INTO project_item (project_id, item_type, github_owner_id, github_owner_login, github_repository_id, github_repository_name, created_at)
SELECT
    p_old.id,
