...
export class StripeInvoice {
  ...
  static fromStripeApi(json: any): StripeInvoice | ValidationError {
    ...
    const invoiceId: StripeInvoiceId | ValidationError =
      StripeInvoiceId.fromStripeApi(json);
    ...
    return new StripeInvoice(
      ...
      json.lines.data.map((line: any) =>
        StripeInvoiceLine.fromStripeApi(customerId, line),
      ),
      json.currency,
      Number(json.total),
