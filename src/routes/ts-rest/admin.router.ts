...
import { getDeveloperProfileRepository, getProjectItemRepository } from "../../db";
      ...
      if (query.verificationStatus) {
        ...
        filteredProfiles = filteredProfiles.filter((profile) => {
          if (profile.profileEntry?.verificationRecords && profile.profileEntry?.verificationRecords.length > 0) {
            const latestProfileRecord = profile.profileEntry?.verificationRecords[0];
          ...
          return profile.projects.some((p: dto.DeveloperProjectItemEntry) => {
            const projectName =
              typeof p.projectItem.sourceIdentifier === "string"
                ...
                : (p.projectItem.sourceIdentifier as any).login || (p.projectItem.sourceIdentifier as any).name;
